    ...
    return (dispatch) => {
        return sessionService.loadUser()  
            .then(currentSession => {
                ...
                .then((response)=>{
                    dispatch({type:'GET_EMPLOYEE_TYPE', data:response.data, loading:false});
                    return response.data
                }).catch(error => {
                    //Error Request
                    if (error.response){
                        let newError = error.response ? error.response.data : 'Something went wrong, please try again.' 
                        dispatch({type:'EMPLOYEE_TYPE_ERROR', error:newError, loading:false});
                    } 
                });
            })
            .catch(error => {
